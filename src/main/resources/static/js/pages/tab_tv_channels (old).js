...
export function changeInspectedChannelListItem(listItem) {
    const selectedChannelName = listItem.attr('data-channel-name');
    const isProgrammeDetailsChild = false; // const isProgrammeDetailsChild = $(event.target).closest('li.channel-programme-details').length !== 0;
    const otherChannelListItemOpen = listItem.siblings('li.programme-list-open').get(0);
    if (otherChannelListItemOpen) {
        tv_channels_list.clearInspectedProgrammeImages(); // Reset inspected programme image
    }
    ...
    if (!isProgrammeDetailsChild) {
        ...
        const newHeight = $('#tab-tv-channels').height() - listItem.find('.channel-banner').outerHeight(); // tv_channels_list.computeMaxHeightOfChannelListItem(listItem);
        const flexParent = listItem.find('.flex-parent');

        const channelDisplayAnimationDelay = utils.cssFloatToInteger(document.app_config.animations.channel_item_display_animation.duration);
